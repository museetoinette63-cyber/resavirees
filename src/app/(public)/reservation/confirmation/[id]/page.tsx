...
export default async function ConfirmationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6 rounded-lg border-2 border-border-warm bg-cream-2 p-8 text-center shadow-sm">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gold-light text-xl text-rust-dark">
        ✓
      </div>
      <h1 className="font-display text-2xl font-semibold tracking-tight text-rust-dark">
      ...
      <Link
        href="/"
        className="inline-block rounded-md bg-rust px-4 py-2 text-sm font-semibold text-cream transition-colors hover:bg-rust-dark"
      >
        Retour à l&apos;accueil
      </Link>
    </div>
